...
import type { DeviceSecurityType, LoginFlowType } from "@thingco/authentication-core";
 ...
 */
const ConfigStateContext = createContext<Configs | null>(null);
 ...
 */
const ConfigUpdateContext = createContext<ConfigUpdateMethods | null>(null);
...
/**
 * A provider that combines the read/write context providers that manage the configuration.
 *
 * @remarks
...
export const ConfigInjector = ({
	...
	const [deviceSecurityType, setDeviceSecurityType] =
		useState<DeviceSecurityType>(initialDeviceSecurityType);
	const [loginFlowType, setLoginFlowType] = useState<LoginFlowType>(initialLoginFlowType);
	const [devToolsStatus, setDevToolsStatus] = useState<DevTools>(
	...
	const [ready, setReady] = useState(isInTestMode);
...
export function useConfigState(): Configs {
	const configs = useContext(ConfigStateContext);
...
export function useConfigUpdate(): ConfigUpdateMethods {
	const configSwitchers = useContext(ConfigUpdateContext);
