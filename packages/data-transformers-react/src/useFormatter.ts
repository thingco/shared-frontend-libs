...
export function useFormatter({
	...
	return {
		averageSpeed: useCallback(
			formatters.averageSpeed({ locale, precision, distanceUnit }),
			[locale, distanceUnit, precision]
		),
		date: useCallback(
			formatters.date({ locale }),
			[locale]
		),
		dateTime: useCallback(
			formatters.dateTime({ locale, clockFormat }),
			[locale, clockFormat]
		),
		distance: useCallback(
			formatters.distance({ locale, precision, distanceUnit }),
		...
		duration: useCallback(
			formatters.duration({ locale, dateDisplayFormat, showSeconds }),
			[locale, dateDisplayFormat, showSeconds]
		),
		speed: useCallback(
		...
		time: useCallback(
			...
			[locale, clockFormat, showSeconds]
		),
	};
}
