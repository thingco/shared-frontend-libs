...
import type { AuthConfig, AuthMachine, InternalAuthContext, InternalAuthEvent } from "./types";
...
const implementations = {
	...
	actions: {
		...
		logChange: (context: any, event: any) => {
			console.log("Changing Auth State", context, event);
		},
	},
};
...
export const machine = model.createMachine(
	{
		...
		states: {
			[AuthStateId.CheckingSession]: {
				entry: ["clearLoginComplete", "logChange"],
				on: {
					SESSION_PRESENT: {
						target: AuthStateId.INTERNAL__deviceSecurityCheck,
						description:
							"With an existing session, user only needs to authenticate against device security to access app",
					},
					SESSION_NOT_PRESENT: {
			...
			[AuthStateId.INTERNAL__loginFlowCheck]: {
				...
				description:
					"Internal state, used for switching flows based on initial config values provided by a client.",
			},
			[AuthStateId.SubmittingOtpUsername]: {
				on: {
					USERNAME_VALID: {
						target: AuthStateId.SubmittingOtp,
						actions: ["assignUser", "assignUsername", "clearError", "logChange"],
					...
					USERNAME_INVALID: {
						...
						actions: ["assignError", "logChange"],
						description:
							"System does not recognise the username. Remain in this state, and show an error.",
					},
				},
				description:
					"OTP username validation. If successfully submitted, the system will send out an OTP and user can move to the OTP input stage. If not, something is wrong with the username.",
			},
			[AuthStateId.SubmittingOtp]: {
				on: {
					OTP_VALID: {
						target: AuthStateId.INTERNAL__deviceSecurityCheck,
						actions: ["clearError", "assignLoginComplete", "logChange"],
						description:
							"The user has completed login and will need to set up device security if required.",
					},
					OTP_INVALID: {
						target: AuthStateId.SubmittingOtp,
						actions: ["assignError", "logChange"],
					...
					OTP_INVALID_RETRIES_EXCEEDED: {
						...
						actions: ["assignError", "logChange"],
						description:
							"The OTP has not been recognised, and the user has exceeded their maximum number of allowed retries.",
					},
					GO_BACK: {
						target: AuthStateId.SubmittingOtpUsername,
						actions: ["clearError", "clearUser", "clearUsername", "logChange"],
						description:
							"The user wishes to reenter their username -- they may have realised they mistyped or used an old accout, for example.",
					},
				},
				description:
					"OTP validation. If successfully submitted, the user has completed login and will need to set up device security if required.",
			},
			[AuthStateId.SubmittingUsernameAndPassword]: {
				on: {
					...
					USERNAME_AND_PASSWORD_VALID_PASSWORD_CHANGE_REQUIRED: {
						...
						actions: ["assignError", "assignUser", "assignUsername", "logChange"],
						description:
							"The username and password combination is correct, but the password is only temporary and they must set up a new, secure password.",
					},
					...
					FORGOTTEN_PASSWORD: {
						...
						actions: ["clearError", "logChange"],
						description: "The user has forgotten their password and wants to request a reset.",
					},
				},
				description:
					"Username/password validation. If successfully submitted, the user has completed login and will need to set up device security if required.",
			},
			[AuthStateId.SubmittingForceChangePassword]: {
				on: {
					PASSWORD_FORCE_CHANGE_SUCCESS: {
						target: AuthStateId.INTERNAL__deviceSecurityCheck,
						actions: ["assignUser", "assignLoginComplete", "clearError", "logChange"],
						description:
							"The user has completed login and will need to set up device security if required.",
					},
					PASSWORD_FORCE_CHANGE_FAILURE: {
						...
						actions: ["assignError", "logChange"],
						description:
							"The password change request has failed -- it could be due to an invalid password, but the reason is not specified. They will need to try again.",
					},
				},
				description:
					"Further username/password validation for users who have a temporary password. If successfully submitted, the user has completed login and will need to set up device security if required.",
			},
			[AuthStateId.ForgottenPasswordRequestingReset]: {
				on: {
					PASSWORD_RESET_REQUEST_SUCCESS: {
						target: AuthStateId.ForgottenPasswordSubmittingReset,
						actions: ["clearError", "assignUsername", "logChange"],
						description:
							"The user confirms they want to reset the password. They only need to enter their username at this point, so will still be able to cancel the request.",
					},
					PASSWORD_RESET_REQUEST_FAILURE: {
						target: AuthStateId.ForgottenPasswordRequestingReset,
						actions: ["assignError", "logChange"],
						description:
							"The request to reset has failed at the first stage. This indicates that the username has not been recognised.",
					},
					GO_BACK: {
						target: AuthStateId.SubmittingUsernameAndPassword,
						actions: ["clearError", "logChange"],
			...
			[AuthStateId.ForgottenPasswordSubmittingReset]: {
				on: {
					PASSWORD_RESET_SUCCESS: {
						...
						actions: ["clearError", "logChange"],
						description:
							"The user successfully confirms the reset. Once this is done, they cannot cancel.",
					},
					PASSWORD_RESET_FAILURE: {
						target: AuthStateId.ForgottenPasswordSubmittingReset,
						actions: ["assignError", "logChange"],
						description:
							"The request to reset has failed at the second stage. They will be allowed to retry or cancel.",
					},
					GO_BACK: {
						target: AuthStateId.ForgottenPasswordRequestingReset,
						actions: ["clearError", "logChange"],
			...
			[AuthStateId.ForgottenPasswordResetSuccess]: {
				on: {
					CONFIRM_PASSWORD_RESET: {
						target: AuthStateId.SubmittingUsernameAndPassword,
						actions: ["clearUsername", "logChange"],
						description:
							"The reset was successful. The user now needs to go through username/password login as normal.",
					},
				},
				description:
					"This state exists to allow contextual text to be displayed to the user, the only action they can take is to confirm they have read the text.",
			},
			...
			[AuthStateId.CheckingForPin]: {
					...
					PIN_IS_SET_UP: [
						{
							...
							description:
								"There is a PIN set up, but the user has also completed login flow. This can occur when the session token has expired. In that case, skip to authenticated.",
						},
						{
							target: AuthStateId.SubmittingCurrentPin,
							description:
								"There is a pin set up, but the user has not completed login flow -- i.e. they had a session. They need to enter their PIN.",
						},
					],
					PIN_IS_NOT_SET_UP: {
				...
				description:
					"Checks whether a user has a PIN stored locally on the device. This stage should ideally not expose UI beyond loading.",
			},
			[AuthStateId.SubmittingCurrentPin]: {
				on: {
					PIN_VALID: {
						...
						actions: ["clearError", "logChange"],
						description:
							"PIN validates against the one stored on the device. User is authenticated.",
					},
					PIN_INVALID: {
						target: AuthStateId.SubmittingCurrentPin,
						actions: ["assignError", "logChange"],
						description:
							"PIN does not validate against the one stored on the device. User must try again.",
					},
					REQUEST_PIN_RESET: {
			...
			[AuthStateId.ForgottenPinRequestingReset]: {
				on: {
					PIN_RESET_SUCCESS: {
						...
						actions: ["clearError", "logChange"],
						description:
							"Resetting PIN. This will log the user out and wipe the current pin. This call should always succeed.",
					},
					PIN_RESET_FAILURE: {
						target: AuthStateId.ForgottenPinRequestingReset,
						actions: ["assignError", "logChange"],
						description:
							"PIN reset failed. This should never happen. If it does, this indicates an underlying problem with the system.",
					},
					CANCEL_PIN_RESET: {
						target: AuthStateId.SubmittingCurrentPin,
						actions: ["clearError", "logChange"],
			...
			[AuthStateId.SubmittingNewPin]: {
				on: {
					NEW_PIN_VALID: {
						...
						actions: ["clearError", "logChange"],
						description:
							"Attempting to set a new PIN. If the check resolves, attempts was successful and they are now Authenticated. ",
					},
					NEW_PIN_INVALID: {
						target: AuthStateId.SubmittingNewPin,
						actions: ["assignError", "logChange"],
						description:
							"Failure setting a new PIN. Failure indicates there may be a problem saving the PIN to storage.",
					},
				},
			},
			...
			[AuthStateId.AuthenticatedChangingPassword]: {
				on: {
					PASSWORD_CHANGE_SUCCESS: {
						...
						actions: ["clearError", "logChange"],
						description:
							"Attempt to change the current password successful. The user may now return to the Authenticated state.",
					},
			...
			[AuthStateId.AuthenticatedChangingPin]: {
				on: {
					PIN_CHANGE_SUCCESS: {
						...
						actions: ["clearError", "logChange"],
						description:
							"Attempt to change the current PIN was successful and the user may return to the Authenticated state.",
					},
					PIN_CHANGE_FAILURE: {
						target: AuthStateId.AuthenticatedChangingPin,
						actions: ["assignError", "logChange"],
			...
			[AuthStateId.AuthenticatedLoggingOut]: {
				on: {
					...
					LOG_OUT_FAILURE: {
						...
						actions: ["assignError", "logChange"],
						description:
							"Logout failed. This should not happen: if it does, then there is an underlying issue with the system.",
					},
					CANCEL_LOG_OUT: {
						target: AuthStateId.Authenticated,
						description:
							"Cancelling at this stage simply takes the user back to the Authenticated state.",
					},
				},
			},
		},
